...
- Tool calls render inline; cost and duration shown per turn
...
- **Permission mode**: how the CLI handles tool permissions. `acceptEdits` is the recommended default. `default` will hang headlessly because there is no permission UI in the plugin yet.
...
Early. Built for one operator, shared because it might help others.

Increment 1 (current): chat panel, streaming markdown, autogrow input, vault scoped cwd.

Planned:

- Increment 2: session continuity, recent chats dropdown, new and delete chat
- Increment 3: save chat as formatted markdown into the vault
- Increment 4: model picker, permission mode quick toggle, per chat inline system prompt, active file context injection
